<template>
  <div class="center no-padding divcol" style="margin-bottom:20px; padding-left: 256px;">
    <section class="section1-acceso">
      <div class="datos-acceso-container">
        <div class="title-morado">
          <p class="datos-acceso-title">
            Acceso
          </p>
          ...
          <v-dialog v-model="dialog" max-width="1600px">
            ...
            <v-card id="dialog-editar-crear">
              <v-card-title>
                <span class="title">Crear Nuevo Acceso</span>
              </v-card-title>
              ...
              <v-card-actions>
                ...
                <v-btn class="btn dialog-btn" @click="createData()" style="background-color:var(--primary)!important;">
                  Guardar
                </v-btn>
              </v-card-actions>
            </v-card>
          ...
          <v-dialog v-model="dialog_editar" max-width="1600px">
            <v-card id="dialog-editar-crear">
              <v-card-title>
                <span class="title">Editar Acceso</span>
              </v-card-title>
        ...
        <div class="data-table-container">
          ...
          <v-data-table :headers="headers" :items="accesoData" :items-per-page="10" :search="search" :footer-props="{
...
export default {
  ...
  methods: {
    ...
    createData() {
      this.$axios.$post('acceso/', this.nuevoRegistro).then(res => {
        console.log(res.data)
        this.nuevoRegistro = {}
        this.accesoData.push(res)
        this.$alert("success", { desc: "Se ha creado un nuevo acceso con éxito", hash: 'knsddcssdc', title: 'Creado' })
      }).catch(err => {
        console.log(err)
      })
    ...
    saveData() {
      ...
      this.$axios.$patch('acceso/' + this.defaultItem.id + '/', formData).then((res) => {
        console.log(res.data)
        this.$alert("success", { desc: "Se ha editado un acceso con éxito", hash: 'knsddcssdc', title: 'Editado' })
        const index = this.accesoData.findIndex((item) => item.id === this.defaultItem.id);
        if (index !== -1) {
          this.$set(this.accesoData, index, { ...this.defaultItem });
        }
      }).catch((err) => {
        console.log(err)
      });
    ...
    deleteItem() {
      this.$axios.$delete('acceso/' + this.defaultItem.id + '/').then((res) => {
        console.log(res.data)
        this.dialogDelete = false
        this.$alert("success", { desc: "Se ha eliminado un acceso con éxito", hash: 'knsddcssdc', title: 'Eliminado' })
        const index = this.accesoData.findIndex((item) => item.id === this.defaultItem.id);
        if (index !== -1) {
          this.accesoData.splice(index, 1);
        }
      }).catch((err) => {
        console.log(err)
      });
...
<style src="~/assets/styles/pages/acceso.scss" lang="scss" />