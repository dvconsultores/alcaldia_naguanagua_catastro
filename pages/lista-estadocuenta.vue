    ...
    <section class="section1-ambito">
      <div class="datos-ambito-container">
        ...
        <div class="data-table-container">
          ...
          <v-data-table :headers="headers" dense :items="EstadoCuentaData" :loading="loading"  :items-per-page="10" :search="search" :footer-props="{
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'Estado Cuenta #', align: 'center', value: 'numero'},
        { text: 'Fecha', value: 'fecha', align: 'center' },
        { text: 'Inmueble', value: 'inmueble.numero_expediente', align: 'center' },
        { text: 'Propietario', value: 'propietario.nombre', align: 'center' },
        { text: 'Trámite', value: 'tipoflujo.descripcion', align: 'center' },
  ...
  methods: {
    ...
    getEstadoCuenta() {
      this.$axios.$get('estadocuenta').then(response => {
        ...
        this.loading = false
        console.log('this.EstadoCuentaData',this.EstadoCuentaData)
      }).catch(err => {
        console.log(err)
      })
    },
    ...
    saveData() {
      if (!this.$refs.defaultItem.validate()) {
        ...
        this.$axios.$patch('estadocuenta/' + this.defaultItem.id + '/', formData).then((res) => {
          console.log(res.data)
          this.$alert("success", { desc: "Se ha editado un EstadoCuenta con éxito", hash: 'knsddcssdc', title: 'Edición de EstadoCuenta' })
          this.getEstadoCuenta()
        }).catch((err) => {
          console.log(err)
        });
