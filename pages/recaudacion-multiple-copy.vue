...
export default {
  ...
  methods: {
          ...
          this.$axios.$get('notacredito/?saldo_gt=0&propietario=' + div2.propietario.id).then(response => {
            if (response[0]) {
              ...
              this.divs.push({
                ...
                monto: parseFloat(this.NotaCreditoData.saldo) < parseFloat(sumaMontos) ? parseFloat(this.NotaCreditoData.saldo) : parseFloat(sumaMontos),
                bloqueado: true,
                bancocuenta: null,
    ...
    generarPDF(Contribuyente,CabeceraPago,DetallePago,Montos,liquidacionDetalleData) {
      ...
      // Convertir nuevamente a cadena sin los corchetes y comillas dobles
      const jsonStringWithoutBrackets = jsonData.map(obj => JSON.stringify(obj)).join('\n');
      console.log('jsonStringWithoutBrackets',jsonStringWithoutBrackets)
    ...
    createPago() {
      ...
      if (this.montoTotalSelectedItem !== null && parseFloat(this.montoTotalSelectedItem) <= this.montoTotalFunc) {
        ...
        for (const divLiq of this.selectedLiq) {
          ...
          //console.log('CabeceraPago',CabeceraPagoApi)
          //console.log('DetallePago',DetallePago.filter(obj => obj.id == div.id) )         
          ...
          /*this.$axios.$get(`liquidaciondetalle/?liquidacion_id=${div.id}`).then(response => {
          ...
          this.ProcesarPago(divLiq.propietario,  divLiq,  DetallePagoApi,  CabeceraPagoApi, data)
          ...
          /*this.$axios.$post('crearPago/', data).then(res => { 
            ...
            this.generarPDF(div.propietario,  div,  DetallePagoApi,  this.liquidacionDetalleData,  CabeceraPagoApi)
    ...
    async ProcesarPago(propietario,  divLiq,  DetallePagoApi,  CabeceraPagoApi,data){
      try {
            ...
            this.generarPDF(propietario,  divLiq,  DetallePagoApi,  CabeceraPagoApi,detail)
    ...
    async ProcesarDetalleLiquidacion(liquidacion_id){
      try {
          const res = await this.$axios.$get(`liquidaciondetalle/?liquidacion_id=${liquidacion_id}`);
          this.liquidacionDetalleData = res
          console.log('liquidacionDetalleData API', this.liquidacionDetalleData)
          //console.log('this.tasaMultaData',this.tasaMultaData)
