    ...
    <section class="section1-modificar-datos">
      <div class="modificar-datos-container">
        <div class="title-morado">
          ...
          <v-dialog v-model="dialog_editar" max-width="1600px">
            <v-card id="dialog-editar-crear">
              <v-card-title>
                <span class="title">Enviar Documento(s)</span>
              </v-card-title>
              <hr>
                ...
                <div class="datos-contribuyente-div">
                  <div class="inputs-datos-container">
                    <div class="divrow center col-mobile" style="max-width:500px; gap:10px; width:100%;">
                      <v-autocomplete v-model="defaultItem.departamento_recibe" :items="perfilData" item-text="nombre"
                        item-value="nombre" label="Envia a:" class="input-dialog">
                      </v-autocomplete>
                    </div>
                    <v-textarea class="input-textarea" v-model="defaultItem.observaciones"
        ...
        <div class="data-table-container">
          ...
          <v-data-table v-model="selectedLiq" :loading="dialogWait" :headers="headers" :items="flujoData" :items-per-page="10"
  ...
  methods: {
    ...
    async saveEstado() {
      ...
      for (const divLiq of this.selectedLiq) {
        try {
          const res = await this.$axios.$patch('flujodetalle/' + divLiq.id + '/', formData); 
          console.log(res.data)
        } catch (err) {
          console.log(err);
        }
        const formDataNew = new FormData()
        formDataNew.append('flujo', divLiq.flujo)
      ...
      this.$alert("success", { desc: "Se ha Enviado el(los) documento(s) con éxito", hash: 'knsddcssdc', title: 'Envio de Documento(s)' })
      this.selectedLiq = []
      this.getFlujo()
