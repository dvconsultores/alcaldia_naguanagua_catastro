...
export default {
  ...
  methods: {
    ...
    async getInmuebleValoracionConstruccion() {
      try {
        if (this.dataValoracionTerreno) {
          const response = await this.$axios.$get(`inmueble_valoracion_construccion2024/?inmueblevaloracionterreno=${this.dataValoracionTerreno.id}`);
    ...
    editTerreno() {
      ...
      this.defaultItem.tipologia_categorizacion = this.dataValoracionTerreno.tipologia_categorizacion
    ...
    saveTerreno() {
      ...
      formData.append('tipologia_categorizacion', this.defaultItem.tipologia_categorizacion)
    ...
    postInmuebleValoracionConstruccion() {
      ...
      const formNewTerreno = {
        ...
        tipologia_categorizacion: this.nuevoRegistro.tipologia_categorizacion,
    ...
    getTipologia() {
      this.$axios.$get('tipologia_categorizacion/?categorizacion=' + JSON.parse(JSON.stringify(this.$store.getters.getExpediente.categorizacion))).then(response => {
...
<style src="~/assets/styles/pages/actualizar-valoracion-economica-2024.scss" lang="scss" />